                ...
                match command {
                    ...
                    Some(Control::AddToQueue(uri)) => {
                        let parsed = SpotifyUri::from_uri(&uri)
                            .map_err(|err| format!("invalid Spotify track URI `{uri}`: {err}"))?;
                        spirc.add_to_queue(parsed)
                            .map_err(|err| format!("could not add track to queue: {err}"))?;
                    }
                    Some(Control::Search(query)) => {
                        spawn_search(session.clone(), query, updates.clone());
                    }
                    Some(Control::RequestArtwork { key, cover_id }) => {
                        request_artwork(
                            session.clone(),
                            key,
                            cover_id,
            ...
            event = player_events.recv() => {
                ...
                match event {
                    ...
                    PlayerEvent::VolumeChanged { volume } => {
                        let _ = updates.send(PlayerUpdate::Volume(volume));
                    }
                    PlayerEvent::ShuffleChanged { shuffle } => {
                        let _ = updates.send(PlayerUpdate::Shuffle(shuffle));
                    }
                    PlayerEvent::RepeatChanged { context, .. } => {
                        let _ = updates.send(PlayerUpdate::Repeat(context));
                    }
                    _ => {}
                }
            }
